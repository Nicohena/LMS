...
const templates: Record<string, string> = {
  welcome: `<h1>Welcome to LMS, {{firstName}}!</h1><p>Your account has been created. You can now log in and start learning.</p><p><strong>Email:</strong> {{email}}</p>{{#if temporaryPassword}}<p><strong>Temporary password:</strong> {{temporaryPassword}}</p><p style="background:#fef3c7;border:1px solid #f59e0b;padding:12px;border-radius:6px;">⚠️ You'll be asked to change this password on your first login. Please log in soon and set a new password.</p>{{else}}<p>Use the password provided by your administrator to log in. You'll be asked to change it on your first login.</p>{{/if}}<p><a href="{{loginLink}}" style="display:inline-block;background:#4f46e5;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;">Log In Now</a></p>`,
